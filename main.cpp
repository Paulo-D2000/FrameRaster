...
struct MyTextureData
{
    ...
    // Host texture pointer
    uint8_t* HostTexurePtr;
...
bool CreateTexture_RGBA(size_t Width, size_t Height, MyTextureData* tex_data)
{
    // Setup dims
    tex_data->Channels = 4;
    ...
    tex_data->HostTexurePtr = new uint8_t[image_size];

    if(tex_data->HostTexurePtr == NULL){
        return false;
    }
...
void UploadTexture_RGBA(MyTextureData* tex_data){
    VkResult err;
    ...
    // Upload to Buffer:
    {
        ...
        check_vk_result(err);
        memcpy(map, tex_data->HostTexurePtr, image_size);
...
void fill(size_t pos, size_t size, uint32_t val, MyTextureData& tex){
    int W = tex.Width, H = tex.Height;
    size_t pos_x = pos % (W/size), pos_y = pos / (W/size);
    uint32_t* ptr = (uint32_t*)tex.HostTexurePtr;
    for (size_t i = 0; i < size; i++)
    {
        for (size_t j = 0; j < size; j++)
        {
            size_t idx = (pos_x*size + i) + (pos_y*size + j) * W;
            idx = idx % (W*H);
            ptr[idx] = val;
        }        
    }   
...
int main(int, char**)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // Draw the texture
        {
            ...
            {
                // Set 4 pixels (2x2) to Yellow if bit is 1 else set to BLue
